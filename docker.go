...
func (p Plugin) writeOutputs(ctx outputContext) error {
	...
	helperBin, err := resolveOutputHelperBin()
	if err != nil {
		return err
	}
	for _, export := range exports {
		...
		cmd := outputCommand(helperBin, args...)
		cmd.Stdout = os.Stdout
		cmd.Stderr = os.Stderr
		trace(cmd)
...
func resolveOutputHelperBin() (string, error) {
	if helper := strings.TrimSpace(os.Getenv("PLUGIN_OUTPUT_HELPER_BIN")); helper != "" {
		return helper, nil
	}
	if helper, err := exec.LookPath("drone-output"); err == nil {
		return helper, nil
	}
	return "", errors.New("outputs requested but output helper was not found; set PLUGIN_OUTPUT_HELPER_BIN or ensure drone-output is on PATH")
}
